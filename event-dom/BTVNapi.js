...
userGrid.addEventListener("click", async (e) => {
    ...
    if (e.target.closest(".btn-update")) {
        await featchUpdate(id);
        return;
    }

    if (e.target.closest(".btn-danger")) {
        await featchdelete(id);
        return;
    }
...
const featchUpdate = async (id)=>{
    editUserId = id
    modal.style.display = "block";
    btnSumbit.innerHTML = "Update"
    btnSumbit.setAttribute("type", "button")
    const res = await featchdelete(id, "GET")
    res.json().then((data)=>{
...
btnSumbit.addEventListener("click", async (e)=>{
    ...
    await featchUpdateUser(editUserId,payload)
    featchUser()
